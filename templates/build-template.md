...
KINH TẾ THÔNG TIN (xem CLAUDE.md `## Content Writing Voice`): mỗi cơ chế/số/quyết định giải thích đúng MỘT lần ở section sở hữu nó (throttle, Trinity balance, một interaction…); chỗ sau chỉ nhắc tên + dẫn ngược, không giảng lại. Build Overview state thesis ở tầm cao — KHÔNG kể trước chi tiết Skill Gems/Tree, cũng KHÔNG bị các section đó chép lại. Verdict thêm phán xét mới, KHÔNG recap. Dẫn mỗi section bằng 1-2 thứ quyết định kết quả; phần phụ gộp một câu. Caveat (verify in-client / PoB2 chưa model) nói một lần ở `pob_coverage` / `confidence_level`, không rải mỗi item.
...
(BẮT BUỘC khi build có section Gear — đây là phần reader cần nhất để tự đi mua, thiếu nó reader không biết slot nào nên hunt mod gì. Bullet 1 dòng/slot cho mọi slot: weapon, off-hand, helmet, body, gloves, boots, belt, amulet, ring ×2, jewel (+ charm).

MỖI dòng MỞ ĐẦU bằng chỉ số cần tìm xếp hạng (cái gì tốt → focus trước), KHÔNG mở đầu bằng tên item — item mình đang chạy chỉ là context ngắn ở cuối dòng. Reader phải biết hunt mod nào dù không có đúng item của mình.
- Slot rare → 2-4 mod cần tìm theo thứ tự ưu tiên, kèm ngưỡng số nếu có breakpoint (vd "Movement Speed 25%+").
- Slot khóa unique → ghi "unique bắt buộc: X vì Y", X → :wiki-link.
Thứ tự ưu tiên chung để xếp hạng trong từng slot: cap res → Life/ES → attribute floor cho gem → damage stat chính → +skill level → reservation/utility → defense. KHÔNG table. Slot vốn là carry/đã có mục cơ chế riêng → tóm chỉ số cần tìm 1 câu + cross-ref.

Ví dụ format (chỉ số dẫn đầu, item theo sau):
- **Boots:** Movement Speed 25%+ → Life/ES → res còn hở → spirit. Mình chạy Atziri's Step (deflection = 50% eva, gánh luôn mobility).
- **Belt:** Life cao → res cứng, ưu tiên res đang dưới cap → charm slot. Slot res chính của build, rare hay flex unique đều được.
- **Amulet:** +minion levels → spirit → crit multi cho carry → res. Anoint The Soul Meridian.)
