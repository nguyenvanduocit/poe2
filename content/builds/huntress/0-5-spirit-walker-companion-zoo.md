...
- **Monkey** (base Alpha Primate) — đường crit. Con monkey có sẵn ~25% base crit; ghép một rare mod **extra crits (+300% crit chance)** là crit cap ngay, không cần node crit nào. Đây là carry single-target burst cao nhất (clip 12M một hit), hướng đi cho ai build quanh crit.
- **Mighty Silverfist** — đường raw damage. Đây là carry phổ biến nhất trên ladder (~23% Spirit Walker field nó), attack pool damage cao và xuất hiện nhiều map nên dễ roll mod tốt. Crit profile thấp hơn monkey (~5% base so với ~25%, đều tooltip read) nên Extra Crits một mình chỉ ra ~20%; lớp thêm :wiki-link{url="https://www.poe2wiki.net/wiki/Critical_Weakness"} — build chạy sẵn :wiki-link{url="https://www.poe2wiki.net/wiki/Malice"} inflict +10 base crit ở 20 stack, cộng trước khi nhân ×4 → (5+10)×4 ≈ 60% — đủ để crit-damage-bonus có nghĩa, không cap-free một-mod như monkey. Chuỗi nhân damage đầy đủ ở [Tame Beast Damage Scaling](/mechanics/0-5-tame-beast-damage-scaling).
...
### Techrod's Revenge + Soul Leader — single-target tech đã thử rồi bỏ
