...
- Tree có 4 loại node: small (stat nhỏ), notable (mục tiêu thật sự trong cluster), keystone (đổi cơ chế hoàn toàn), jewel socket (gắn jewel tháo tự do).
- Class xuất phát gần vùng attribute tương ứng — chọn sai class phải đi qua 5-10 small passive thêm chỉ để đến cluster cần.
- Đường đi quan trọng hơn điểm đến — một notable cần qua 6 small passive thực sự tốn 7 điểm, không phải 1.
- Keystone luôn có hai chiều: lợi ích và đánh đổi — đọc kỹ cả hai trước khi lấy.
- Plan trước trong Path of Building tránh respec tốn gold.
...
Tree có bốn loại node. **Small passive** cung cấp stat nhỏ — 5% increased damage hay +10 life — và nằm khắp nơi như đường nối giữa các node lớn hơn. Mỗi node tốn một điểm trong khi stat nhận được rất thấp, nên đừng lấy small passive chỉ vì thấy "được gì đó".

**Notable passive** mới là điểm đến thực sự trong một cluster: node này to hơn rõ rệt, stat lớn hơn, thường là lý do đi vào khu đó ngay từ đầu. Mọi cluster đều có một hoặc vài notable ở trung tâm, bao quanh bởi small passive dẫn đường vào.
...
Nếu chơi build evasion-based thì Huntress hay Ranger xuất phát gần đúng cluster. Nếu chọn class Strength nhưng lại đi lấy energy shield ở vùng Intelligence xa phía bắc, sẽ tốn thêm 5-10 điểm chỉ để "đi đường" qua small passive không liên quan — tiết kiệm được số điểm đó bằng cách chọn class phù hợp ngay từ đầu.

## Đường đi quan trọng hơn điểm đến

Điều người mới hay quên: không thể nhảy thẳng đến một notable hay keystone ở xa mà phải đi qua từng node một. Mỗi small passive trên đường đi tốn một điểm. Nếu một notable cần đi qua sáu small passive để đến, thực sự chi bảy điểm cho nó, không phải một.
...
Mỗi level nhận được một passive point. Quest trong campaign reward thêm điểm — tổng cộng có thể kiếm đến 124 điểm ở max level, nhưng thực tế khi đang leveling làm việc với 60-80 điểm. Mỗi điểm đều đắt.

## Plan trước trong Path of Building tránh respec tốn kém

**Path of Building** (PoB) là tool offline cho phép simulate build trước khi commit điểm thật trong game. Import passive tree của bất kỳ character nào từ pobb.in, mở tab Passive Tree, và allocate thử. PoB tính tổng stat, DPS dự kiến, và defensive layer ngay khi thay đổi node.
