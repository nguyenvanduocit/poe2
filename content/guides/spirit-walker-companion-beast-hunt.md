...
- Companion carry nghĩa là damage nhân vật chính là damage của một con beast tame bằng Tame Beast rồi bind làm companion; min-max là ráp pipeline săn rồi chồng multiplier, không xoay skill tay.
- Tame Beast giữ tối đa 4 regular monster modifier (random nếu con có hơn 4, không giữ Essence hay Azmerian wisp mod); reservation scale theo mod count nên đừng nhồi mù.
- Con carry crit chia hai tier: Alpha Primate (monkey) ~25% base cap crit chỉ trên một Extra Crits, Silverfist/Zekoa ~5% cần thêm Critical Weakness (có sẵn qua Malice) mới tới ~60%.
- Bốn bucket nhân damage compound: gem level (~60% more @ L20, ưu tiên số một), crit từ monster modifier, Pain Offering package, Parry debuff (~100% magnitude từ node cây, tới ~150% với Dunkelhalt, chỉ cho attack).
- Ưu tiên mod khi soi boss: Extra Crits số một tuyệt đối, rồi tới Enraged/Hasted, Soul Eater, Extra elemental damage.
- Săn beast: Untainted Paradise cho volume, essence reset cho đúng base, camp cho base phổ biến; tier map không đổi base stat con tame nên chạy tier thấp cho an toàn.
- Nhồi mod bằng tablet stacking (Cruel Hegemony + "of Contest", ba slot mở từ waystone 6-mod) và chain boss bằng Rite of the Nameless cho ~5 attempt mỗi key; soi-rồi-reroll trên waystone 4-mod (2 revive) an toàn hơn run 6-mod one-shot (0 revive).
...
**Một: gem level.** Đó là more-multiplier nằm sẵn trên skill: minion more-damage leo theo gem level, ~60% more ở Level 20 và lên ~79% ở Level 30, cộng flat 25% more dành riêng cho Tamed Beast ở mọi gem level. Nó chạm mọi con beast bất kể base, nên +level minion là upgrade ưu tiên số một, lấy từ amulet, sceptre, helmet (suffix "+# to Level of all Minion Skills"). Jewel **không** cấp gem level; đóng góp minion của jewel nằm ở dòng increased damage và crit-damage-bonus suffix ("of Gripping").
...
Tame Beast scaling giữ nguyên: minion more-damage theo gem level (~60% @ L20, ~79% @ L30) cộng 25% more của Tamed Beast ở mọi level; patch không đụng companion (Trusted Kinship, Extra Crits, Critical Weakness, Parry, Pain Offering đều y nguyên). The Natural Order possess con Unique tame bằng Azmeri Spirit đổi mỗi 20s — layer riêng, bốn rare mod giữ lúc bắt không rotate.
