...
Có một mâu thuẫn nguồn phải ghi thẳng vì nó chưa được giải. Parse poe2db cho base engine của cả :wiki-link{url="https://www.poe2wiki.net/wiki/Alpha_Primate"} lẫn :wiki-link{url="https://www.poe2wiki.net/wiki/Mighty_Silverfist"} đều là **5% crit / +30% crit-damage-bonus** — nhưng 5%/+30% là default chung của gần như mọi monster nên đây là weak evidence, không phải override riêng từng con (parse base-skill-value không bắt được bonus áp lúc tame). Ngược lại, một guide CN độc lập report thẳng con **Mighty Silverfist** — đúng con ở trên đang xếp ~5% raw-damage — đạt **25% crit + 250% crit damage**, tức gán hướng high-crit cho Silverfist chứ không phải monkey. Hai điểm khiến claim này không thể bỏ qua: nó là nguồn thứ hai độc lập report crit cao trên một con ape, và con số **250% crit-damage KHÔNG sinh ra từ Extra Crits** (mod đó chỉ cho +300% crit *chance*, không chạm crit damage) — nên nó không phải artifact của mod đã roll bị đọc nhầm thành base. Khả năng cao nhất là tooltip của companion **đã summon** khác hẳn base wild-monster. Tóm lại: ape nào mới là con high-crit, và base crit/crit-damage thật là bao nhiêu, vẫn chưa chốt giữa các nguồn — đừng dựng math cứng lên một con số cụ thể trước khi đo trong client.
...
- **2026-06-04** — Fold mâu thuẫn nguồn crit (seed: guide CN Ash Rose, caimogu.cc). Parse poe2db cho base cả hai ape = 5%/+30% nhưng đó là default engine của hầu hết monster (weak evidence, không phải override riêng). Guide CN report chính **Mighty Silverfist** đạt 25%/250%, gán high-crit cho Silverfist ngược split monkey-25%/Silverfist-5%; con 250% crit-damage KHÔNG sinh từ Extra Crits (mod chỉ +300% crit chance) nên không phải mod đọc nhầm → nghi summoned-tooltip khác wild-base. Không rewrite chain — thêm data point vào section crit + sharpen verify-list (2): đọc summoned-tooltip trên tame trắng cả hai ape mới phân được. CN name: Alpha Primate = 灵猿族长, Mighty Silverfist = 神威银拳/猿神.
...
- **Cần log khi vào league:** (1) con Unique tame (qua The Natural Order) có rotate rare monster modifier mỗi ~20s không, và pool modifier thực gồm gì; (2) số base-crit + crit-damage-bonus thật của monkey / Silverfist — đọc tooltip con companion **đã summon** trên một con tame **trắng** (chưa roll mod) cho cả hai ape, vì poe2db base (5%/+30%) là default engine còn creators report 25% (và guide CN report 25%/250% trên chính Silverfist) → chỉ summoned-tooltip mới phân được tame-transform có cộng crit/crit-damage không, và ape nào là con high-crit; (3) crit chance đo thực tế trên con carry sau khi Extra Crits + Critical Weakness online; (4) Parried Debuff Magnitude tổng đạt được qua weapon-set có chạm ~100% more không.

## Relationships

- **part_of** [Tame Beast Companion Carry Spirit Walker](/builds/huntress/0-5-spirit-walker-companion-carry) — build dùng chuỗi multiplier này để max con carry chính; chi tiết loop chơi (snapshot Parry, Reputation) và lớp phòng thủ Loyalty/Romira's Requital sống ở đó.
