...
- **Cần log khi vào league:** (1) double-effect node của Jado có apply cho dòng unique-monster không; (2) cơ chế reset/reroll modifier của boss khi respawn-at-checkpoint; (3) base stats con beast có tier-invariant không (T1 = T15) — low-tier hiện chạy vì survivability, cơ chế chưa confirm; (4) số base-crit thật của monkey (~25%) và Silverfist (~5%) đọc trong client — cả hai là tooltip read, chưa có nguồn authoritative.
