...
Tree 0.5 có 85 node cấp faster start of Energy Shield Recharge, chủ yếu là các small node 4% và 6%. Rate gần như bị tước khỏi small node, chỉ còn sót lại ở một số notable cụ thể và gear. Runic Ward xuất hiện cùng patch như lớp backup khi life chạm 1, buộc build phải nghĩ lại cách kết hợp recovery với các layer khác.
...
Ghost Dance tạo lớp ES regen độc lập với recovery rate thông thường. Mỗi khi mất Ghost Shroud, nhân vật nhận 2% Evasion Rating dưới dạng ES regen mỗi giây. Với 10k Evasion, đây là khoảng 200 ES/s ổn định, không bị ảnh hưởng bởi các nerf recovery trên tree.
...
Pattern recharge spam giữa hai hit boss gần như chết. Với TTF tăng đáng kể so với 0.4, boss có downtime dưới 3 giây sẽ không cho đủ thời gian full refill.
...
Sai: Coi Patient Barrier là notable pool "sạch" như 0.4.  
Đúng: Phải chấp nhận 20% slower start và bù bằng Runic Ward mạnh.  
Lý do: Downside này làm TTF tệ hơn nếu không có Ward backup.

Sai: Path sâu vào cluster recovery cũ (Convalescence + Essence Infusion + Rapid Recharge).  
Đúng: Chỉ lấy Rapid Recharge nếu rẻ, ưu tiên Deflection notable trên nhánh Evasion.  
Lý do: Cluster cũ giờ trả rất ít value so với chi phí point.

Sai: Bỏ qua Verisium Runeforging vì nghĩ là league mechanic phụ.  
Đúng: Đây là layer phòng thủ chính bù đắp cho recovery yếu.  
Lý do: Build không có Ward sẽ chết nhanh hơn rõ rệt ở high tier map và boss.
