...
- Bật delirium mirror ở CUỐI map, không phải đầu — build yếu vẫn ăn đủ reward các mốc mà khỏi clear cả map dưới sương.
- Vốn chỉ 3 tablet Delirium rẻ; sàn income là Simulacrum Splinter, currency thanh khoản dày nhất chợ.
- Tablet chính: mod more-likely-spawn-unique-boss (~50-70 ex) → đám unique boss cuối map nhả Liquid Emotion cao cấp. Build chưa hạ nổi thì đổi mod stack-size splinter (~20-30 ex).
- Thứ tự bắt buộc: dọn unique boss → giết map boss → RỒI mới end counter. Đảo là mất reward tầng cuối.
- Tính đến 2026-06-19, build zoom 1 map/phút ăn ~12-18 div/giờ; build 2 phút/map chia đôi.

Boss rush Delirium để gặt splinter: rush thẳng boss mỗi map, để dành :wiki-link{url="https://www.poe2wiki.net/wiki/Delirium"} bật ở cuối map rồi gặt :wiki-link{url="https://www.poe2wiki.net/wiki/Simulacrum_Splinter"}. Tier A, investment Low — cửa vào tốt nhất cho build chưa đủ mạnh cày 200% Delirium.
...
Delirium subtree lấy hết — engine fog viết kín ở [Delirium và Trial of Madness](/guides/0-5-delirium-trial-of-madness); là thứ duy nhất gate income splinter. Node `5% chance Grand Mirror appear on nearby map` gieo mirror cho đường income thứ tư.

Hai notable rework ở 0.5.3:

- **You can't just wake up from this one.** → pick **Escalating Threats**: phần "extra Modifier vào area" áp lên cả vùng fog Grand Mirror map thường, không chỉ Simulacrum. Không chạy Simulacrum thì các pick còn lại đều dead.
- **Are you sure you want to do that?** → cho "Areas with Grand Mirrors also have a Delirium Mirror". Vẫn lấy vì cú extra mirror làm fog dày hơn ngay tại map, nhưng đừng kỳ vọng nhân đôi tablet (đã bỏ cú tablet-double-effect cũ).

Phần cây còn lại gom generic áp mọi map (rarity, rare monster, magic pack size) cộng hai node vắt value tablet: **chance to not consume tablets** và **+1 maximum modifier on tablet**.

## Master chọn ai

Ba master swap tự do trước mỗi map:

- **Jado** — chính, lấy cụm bốn node: **Partial Translations** (0-40% effect explicit modifier trên tablet), **Unforeseen Threats** (5% lộ Anomaly Map gần, loot phụ miễn phí), **Untold Histories** (35% chance Lineage Support), **Unexpected Missions** (corrupt waystone mở thêm map area + 1 mod — alch-and-go nên corrupt sạch waystone trước khi cắm).
- **Hilda Ancient Inscriptions** (+25% effect mỗi loại tablet) khi muốn tối đa tablet effect thuần.
- **Doryani Hidden Patterns** (10% tự mở map liền kề) khi ưu tiên travel về Citadel.

## Chạy boss rush

- Setup: 3 tablet Delirium, waystone 6 mod, subtree Delirium hết, Jado. Rào build: đủ DPS hạ map boss + đám unique boss cuối, đủ sống rush qua rare — companion clear chạy thoải mái. Fog chỉ bật cuối map nên không cần giết kịp quái delirious.
- Vào map, delirium mirror spawn ở đầu — **đừng bấm**.
- Rush tới checkpoint phòng boss, skip quái dọc đường (giết rare nếu tiện).
- Tới checkpoint boss thì quay ngược về đầu, bấm mirror, đứng chờ thanh fog đầy (~10-15s).
- Teleport về checkpoint cuối — reward các mốc spawn trước mặt.
- Dọn đám unique boss trước (oil cao cấp), giết map boss, **rồi mới** end delirium counter. Đảo thứ tự = mất nguyên reward tầng cuối gồm cú splinter to.
- Profit/giờ = clear speed × movement speed, vì income tính theo map.

## Kinh tế

Số poe2scout 2026-06-19 (patch-day 0.5.3, re-check trước session dài), 1 :wiki-link{url="https://www.poe2wiki.net/wiki/Divine_Orb"} ≈ 191 ex:
...
- EV/map: 0,22 div riêng splinter × 60 map/giờ ≈ 13 div/giờ (build zoom 1 map/phút), lên **12-18 div/giờ** khi cộng raw currency + oil; build 2 phút/map chia đôi.
- Cost ~1 div/cọc-3-tablet chia 10 charge → gần như bằng không.
