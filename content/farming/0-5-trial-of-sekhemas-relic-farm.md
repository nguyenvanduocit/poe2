...
```
expected_profit_per_run =
  cache_floor      (Arcanist currency + Cartographer waystone + Jewel cache + Relic cache)
  + relic_resale   (relic non-unique rớt → reforge 3:1 → bán dòng honour-res/key tốt)
  + zarokh_jackpot (unique relic + Zarokh's Reliquary Key, chỉ tầng bốn, phương sai cao)
  − key_cost       (Djinn Barya area level 75+)
  − relic_setup    (đầu tư honour-res + key-quantity relic, amortize qua nhiều run)
```
