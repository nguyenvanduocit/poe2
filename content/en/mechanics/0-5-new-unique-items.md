...
0.5 adds 42 new unique items, and the full poe2db list shows the league's direction more clearly than any patch note line: most new items attach to three axes — companion (Spirit Walker), Remnant/Runic Ward (the crafting league mechanic), and mana-as-resource. At the point before the league goes live (25/05, launch ~29/05), only **23/42 items have fully revealed stats** on poe2db; the remaining 19 are datamine shells (name + base type, mods not yet populated). Worth hunting in week one are the items with revealed numbers that define an archetype; the rest have to wait for in-game logging.
...
Wrong — writing a build-around one of the 19 datamine shells because the name sounds good. Right — wait for stats to populate before committing. Reason — there are no mods to build on yet, any plan based on an empty page has nothing to verify against.
