...
//
// COST TIERING (project override) — agents no longer inherit the (possibly Opus)
// session model. Each phase is pinned to the cheapest model that preserves quality:
//   search/fetch = haiku (simple retrieval + schema-bound extraction, ~20 agents = the bulk)
//   scope/verify/synthesize = sonnet (angle selection, adversarial skepticism, final report)
// Tune all five in one place via the MODEL map below.
const MODEL = {
  scope: "sonnet",
  search: "haiku",
  fetch: "haiku",
...
const scope = await agent(
  ...
  { label: "scope", schema: SCOPE_SCHEMA, model: MODEL.scope }
)
if (!scope) {
...
const searchResults = await pipeline(
  ...
  angle => agent(SEARCH_PROMPT(angle), {
    label: "search:" + angle.label, phase: "Search", schema: SEARCH_SCHEMA, model: MODEL.search
  }).then(r => {
    if (!r) return null
...
const report = await agent(
  ...
  { label: "synthesize", schema: REPORT_SCHEMA, model: MODEL.synthesize }
)
