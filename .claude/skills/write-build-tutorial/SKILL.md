...
- `$topic_or_path` — Một trong:
  ...
  - **Topic** ngắn (vd `spectre necromancer`) → skill glob lookup. File chưa tồn tại → REFUSE, gợi ý tạo file mới theo template `templates/build-template.md` trước. Skill KHÔNG tạo file mới.
...
8. **Kinh tế thông tin — mỗi fact một chỗ, dẫn bằng cái quan trọng, không recap** — Mỗi cơ chế/số/quyết định giải thích đầy đủ đúng MỘT lần ở section sở hữu nó (throttle, một interaction, một defense layer…); chỗ sau chỉ nhắc tên + dẫn ngược, KHÔNG giảng lại. Build Overview state thesis tầm cao, KHÔNG kể trước chi tiết Skill Gems/Tree và KHÔNG bị chúng chép lại. Mỗi section dẫn bằng 1-2 thứ quyết định kết quả (carry gem, key item), phần phụ gộp một câu — đừng cho mọi gem/node/jewel đoạn dài ngang nhau. Verdict thêm phán xét mới, KHÔNG bullet-recap. Caveat (verify in-client / PoB2 chưa model) nói một lần ở `pob_coverage`/`confidence_level`, không rải mỗi item. Self-test: số/cơ chế nào được *giải thích* ở >1 chỗ → gộp còn một. Ref: CLAUDE.md `## Content Writing Voice` → "Kinh tế thông tin".
...
### 6. Validate
Frontmatter khớp schema `content.config.ts` — chạy `bun run generate` để verify. Warning OK nhưng show user.
