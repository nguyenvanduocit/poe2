...
> Note: `category: "map"` is the API category umbrella for Waystones on trade (legacy naming carried over from POE1). The on-site filter is labelled "Waystone Tier".
...
```bash
cd .claude/skills/map-mod-filter
