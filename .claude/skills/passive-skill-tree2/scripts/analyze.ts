 ...
 * Data source: poe2db.tw mirror at data/poedb2/<patch>/passive-skill-tree/data_us.json
 *
 * Commands:
 ...
 *
 * Usage:
 *   bun .claude/skills/passive-skill-tree2/scripts/analyze.ts lookup "Spirit"
 *   bun .claude/skills/passive-skill-tree2/scripts/analyze.ts classes
 *   bun .claude/skills/passive-skill-tree2/scripts/analyze.ts search-stats "minion damage"
 *   bun .claude/skills/passive-skill-tree2/scripts/analyze.ts ascendancies "Witch"
 *
...
// Data source: poe2db.tw mirror, patch-versioned at <project-root>/data/poedb2/<patch>/passive-skill-tree/data_us.json
...
// (Martial Artist, 0.5).
//
// Refresh: ./.claude/skills/poedb2/scripts/download.sh <patch>  (full poedb2 mirror)
...
// POE2 0.5.x maps to internal "4.4" (per data/poedb2/0.5.0/passive-skill-tree/meta.json caveat:
// upstream "0.5" returns 404). Override via --source-version <X.Y> when poe2db bumps.
const POEDB_ROOT = resolve(dirname(new URL(import.meta.url).pathname), "../../../../data/poedb2");
...
// Resolve the newest patch directory under data/poedb2/. Returns "0.5.0" style patch label.
...
function loadTreeData(): SkillTreeData {
  ...
  if (!patch) {
    console.error("No patch data found under data/poedb2/.");
    console.error("Run: ./.claude/skills/poedb2/scripts/download.sh <patch>   (full mirror)");
    console.error("Or:  bun .claude/skills/passive-skill-tree2/scripts/analyze.ts --force-update   (tree-only direct fetch)");
    process.exit(1);
  }
...
Options:
  ...
  --patch <X.Y.Z>             Use specific POE2 patch (default: newest under data/poedb2/)
