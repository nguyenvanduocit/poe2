# passive-skill-tree2/scripts — data layer notes

**Trạng thái: LIVE.** `analyze.ts` đã implement đầy đủ và đọc data từ poedb2 mirror — cùng command surface với `/passive-skill-tree1`. Không còn là scaffold.
...
- `analyze.ts` auto-resolve patch cao nhất dưới `data/poedb2/`, override bằng `--patch <X.Y.Z>`.
...
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts types --force-update

# Full poedb2 mirror (HTML + JSON + mọi thứ dưới /us/):
./.claude/skills/poedb2/scripts/download.sh 0.5.0
```
...
- poedb2 mirror downloader: `.claude/skills/poedb2/scripts/download.sh`
- POE2 0.5 patch notes: `data/release-notes/poe2/Version_0.5.0.md`
- Last fetch evidence: `data/poedb2/0.5.0/passive-skill-tree/meta.json`
