...
version: 1.0.0
license: MIT
compatibility: Requires bun runtime. Reads patch-versioned poe2db mirror under data/poedb2/<patch>/passive-skill-tree/data_us.json.
metadata:
  author: poe-aio
  data-source: poe2db.tw
  status: live
allowed-tools: Bash(bun scripts/passive-skill-tree2:*
---
...
```bash
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts lookup <query>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts nearby <id> [count]
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts distance <id1> <id2>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts path <from> <to>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts analyze <nodes...>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts validate <class> <nodes...>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts search-stats <text>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts suggest <start> <points>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts types
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts classes
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts ascendancies <class>
bun .claude/skills/passive-skill-tree2/scripts/analyze.ts export
```

Command surface matches `/passive-skill-tree1` byte-for-byte; only the underlying class roster + node id space differ.

**`analyze`** does real stat aggregation: it parses the number out of every stat line, sums lines that share the same wording (template), and groups totals by category (life / energy shield / mana / spirit / resistance / defence / attributes / damage / critical / speed / recovery). It is additive bucketing, not a multiplicative damage engine — for true DPS/EHP use `/pob2`.
...
| `--patch <X.Y.Z>` | Use specific POE2 patch label (default: newest semver dir under `data/poedb2/`) |
...
- Local cache: `<project-root>/data/poedb2/<X.Y.Z>/passive-skill-tree/data_us.json`
- Auto-resolve: skill scans `data/poedb2/` and picks the highest semver folder. Override with `--patch <X.Y.Z>`.
- Refresh options:
  - Full mirror (HTML + JSON + everything under `/us/`): `./.claude/skills/poedb2/scripts/download.sh <patch>`
  - Tree-only direct fetch (this skill): `bun .claude/skills/passive-skill-tree2/scripts/analyze.ts <cmd> --force-update`

**poe2db version quirk:** the URL counter does NOT track POE2 patch numbers. POE2 0.5.x maps to internal `4.4` (per `data/poedb2/0.5.0/passive-skill-tree/meta.json` caveat — `0.5` returns 404 upstream). When poe2db bumps, pass `--source-version <X.Y>` once and update `POEDB_DEFAULT_SOURCE_VERSION` in `scripts/analyze.ts`.
...
- Evidence của last data fetch: `data/poedb2/0.5.0/passive-skill-tree/meta.json`.
