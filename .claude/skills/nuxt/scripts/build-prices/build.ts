#!/usr/bin/env bun
//
// Collapse `data/price-history/poe1/price-history.json` (~2.7 MB, time series) into
...
// snapshots live under `data/price-history/poe1/daily/` for audit only.
const SOURCE = resolve(REPO_ROOT, 'data/price-history/poe1/master.json')
const APP_CONFIG = resolve(REPO_ROOT, 'app/app.config.ts')
const OUTPUT = resolve(REPO_ROOT, 'app/composables/data/prices.generated.ts')

// Pull `currentLeague` from `app/app.config.ts` via regex so we never drift
// out of sync with the site config when the league rolls over.
function readCurrentLeague(): string {
  const cfg = readFileSync(APP_CONFIG, 'utf8')
  const m = cfg.match(/currentLeague:\s*['"]([^'"]+)['"]/)
  if (!m) throw new Error(`Cannot find currentLeague in ${APP_CONFIG}`)
  return m[1]!
}
