# farming-strategy2/scripts — TODO
...
- **Fallback / cross-check:** trade2 API qua CDP Relay (xem `/trade2` skill). Trade2 cho real-time listing nhưng rate-limit chặt — chỉ dùng cho spot-check không cho bulk snapshot.
...
bun .claude/skills/farming-strategy2/scripts/market-snapshot.ts --top 30
...
bun .claude/skills/farming-strategy2/scripts/analyze.ts --strategy ocean-exploring --maps-per-hour 8
...
6. [ ] Test với current character POE2 0.5 (TBD) — fetch real character envelope qua `/pob2`, cross-check build_requirements field thực tế tank được strategy.
