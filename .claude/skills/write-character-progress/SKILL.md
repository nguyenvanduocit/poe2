...
(Required core 5: Snapshot · Current Goals · Priority Actions · Gear Summary · Progress Log. Optional: Challenge Tracking. CẮT section không dùng, ĐỪNG pad. Heading literal để giữ schema nhất quán.)
...
2. **## Snapshot** — Defenses + offense tại thời điểm fetch. Heading literal là `## Snapshot` (không parenthetical date) để giữ schema nhất quán. Note timestamp ở dòng đầu section: `*Live fetch: YYYY-MM-DD*` (italic, dưới heading). Format khuyến khích bullet stat-dump (đây là exception cho prose-first vì purely data points):
   - **ES / Life:** ...
