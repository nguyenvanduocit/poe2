...
## Guide mở đầu bằng TL;DR

Mọi doc trong `content/guides/` mở đầu bằng section `## TL;DR` ngay sau H1 — 3-7 bullet plain-text gói các quyết định + số then chốt để người đọc scan trong 10 giây rồi mới đọc thân bài. Quy ước:

- **Bullet là kết luận, không phải mục lục** — mỗi bullet nói *cái gì / bao nhiêu / chọn gì*, KHÔNG phải tên section ("phần 1 nói về X" là sai).
- **KHÔNG wiki-link trong bullet TL;DR** — giữ list sạch để scan; body vẫn giữ `:wiki-link` first-mention cho mọi game term.
- **Thân bài KHÔNG lặp nguyên văn bullet** — TL;DR tóm, body giải thích *why* (kinh tế thông tin: mỗi fact giải thích một lần ở section sở hữu nó).
- **Owner-voice + banned-lexicon** áp cho bullet như mọi prose khác (xem `templates/voice-lexicon-vi.md`).
- **Exception**: doc dùng `patch-notes-template` (archive verbatim GGG) KHÔNG có TL;DR — digest đã sống ở doc overview riêng.

Template (`guide-template.md`, `mechanic-template.md`, `item-template.md`) và skill `/write-mechanic-tutorial` đã encode TL;DR là required-core opener.
